...
Status: v1. This document defines the protocol and behavior implemented with PostgreSQL storage.

The relay started on SQLite and now uses PostgreSQL; the HTTP protocol, credential rules, task lifecycle, and delivery guarantees are unchanged.
...
- **PostgreSQL:** persists agents, tasks, and delivery attempts. Row locks (`FOR UPDATE SKIP LOCKED` for claims, a per-task `FOR UPDATE` for heartbeat, terminal submission, and recovery) coordinate concurrent claims across API/worker processes.
...
Claiming atomically changes the task to `processing` and creates a delivery attempt with a new claim token. Concurrent workers cannot successfully claim the same task while that lease is active. The claim runs in one transaction that selects the oldest queued task with `FOR UPDATE SKIP LOCKED`, rather than using process-local locks.
